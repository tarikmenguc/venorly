...
def fetch_trending_models_node(state: AgentState) -> AgentState:
    ...
    try:
        results = get_models_store().similarity_search(query, k=15)
    ...
    sample_size = min(6, len(all_models))
...
(Sadece listeyi yaz, başka açıklama ekleme)"""
    ...
    try:
        response = get_llm().invoke([HumanMessage(content=clustering_prompt)])
...
def generate_opportunity_node(state: AgentState) -> AgentState:
    """Tüm verileri birleştirerek yapılandırılmış fırsat raporu üretir.
    Faz 2: Artık rakip şikayetleri de rapora dahil."""
    print("[Agent] Node 5 → generate_opportunity")
...
Rakip Uygulamaların Şikayetleri (G2, Capterra, Reddit, Trustpilot):
{complaint_clusters}
"""
    else:
        complaints_section = ""
    ...
    if store_clusters:
        store_section = f"""
Gerçek Kullanıcı Yorumları (Play Store + App Store, 1-2 yıldız):
{store_clusters}
"""
    else:
        store_section = ""
    ...
    perspectives = [
        ...
        "Eğitim sektörü (öğretmenler, online kurs yapımcıları, öğrenciler)",
        "E-ticaret (küçük mağaza sahipleri, dropshipper'lar)",
        "Hukuk sektörü (avukatlar, hukuk büroları, noterler)",
        "Gayrimenkul (emlakçılar, inşaat firmaları)",
        "İçerik üreticileri (YouTuber'lar, podcast'çiler, blogger'lar)",
        "Freelancer'lar (tasarımcılar, yazarlar, geliştiriciler)",
        "Restoran ve yeme-içme sektörü",
        "Fitness ve spor antrenörleri",
        "Muhasebeciler ve mali müşavirler",
        "Fotoğrafçılar ve videograflar",
        "HR ve insan kaynakları uzmanları",
    ...
    prompt = f"""Sen bir Micro-SaaS ve yan gelir (side-hustle) strateji uzmanısın.
Amacımız, milyar dolarlık devleri (örn. Midjourney, Cursor) kopyalamak ASLA değil.
Biz, dar bir hedef kitleye (niş) odaklanan, aylık $1,000 - $50,000 arası kazanan kârlı Mikro-SaaS fikirleri arıyoruz.

Kullanıcı kategorisi: {state['user_category']}

🎲 Bu sefer şu perspektiften düşün: {chosen_perspective}
(Bu perspektifi kesinlikle kullanmak zorunda değilsin, ama öncelikli olarak bu yönden fırsat ara.)

Trend AI Modeller (bu hafta popüler):
{models_text}

Bu alanda para kazanan uygulamalar (TrustMRR + ProductHunt):
{apps_text}
{complaints_section}
{store_section}
Görevin:
1. Yukarıdaki verilerden spesifik bir Micro-SaaS fırsatı belirle.
2. {"Rakip şikayetlerini analiz ederek hangi boşluğu doldurabileceğini somut örneklerle açıkla." if complaint_clusters else "Potansiyel boşlukları belirle."}
3. {"Store yorumlarındaki gerçek kullanıcı acılarını referans al." if store_clusters else ""}
4. YARATICI ve ÖZGÜn bir fikir üret. Klişe fikirlerden kaçın.

Çıktı formatı (Türkçe, emoji kullan):

🔥 NİŞ FIRSAT: [Çok spesifik kısa fikir başlığı]
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
📦 Kullanılacak Model: [Model adı ve niş için neden uygun olduğu]
🎯 Odaklanılacak Niş: [Hangi dar kitle]

💰 Pazar Mantığı:
   [Listeden küçük çaplı uygulamaları örnek göstererek o kitlenin para ödemeye yatkınlığını açıkla.]

❌ Rakip Boşlukları (Gerçek Kullanıcı Şikayetleri):
   [{"Yukarıdaki şikayet kümelerinden ve store yorumlarından referans vererek somut boşlukları listele." if (complaint_clusters or store_clusters) else "Potansiyel zayıf noktaları belirle."}]

💡 Fırsat (Micro-SaaS Özeti):
   [Tam olarak ne yapılacak, nasıl pazarlanacak, önerilen aylık abonelik ücreti]

🔗 [Model URL]
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

Sadece raporu yaz, başka açıklama ekleme."""
...
def build_graph():
    ...
    graph.add_edge("cluster_store_problems",     "generate_opportunity")
